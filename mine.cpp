...
public:
    ...
    int miniMax(int cur_depth,int depth);
};
  ...
  /*  if(curState.disc_count[1]+curState.disc_count[2]<10)
    ...
    for(int depth=0;;depth++)
    {
        ...
        for(int i=0;i<n_valid_spots;i++)
        {
            ...
            temp.put_disc(x);
            curStateval[i]=temp.miniMax(0,depth);
        }
...
int OthelloBoard::miniMax(int cur_depth,int depth)
{
    if(cur_depth==depth)
    {
        int total=0;
        ...
        for(int i=0;i<next_valid_spots.size();i++)
        {
            ...
            //C 
            else if(next_valid_spots[i]==Point(0,1)||next_valid_spots[i]==Point(1,0)||next_valid_spots[i]==Point(6,0)||next_valid_spots[i]==Point(1,7)||
                    next_valid_spots[i]==Point(7,6)||next_valid_spots[i]==Point(6,7)||next_valid_spots[i]==Point(0,6)||next_valid_spots[i]==Point(7,1))
            {
                C_num++;
            }
            //X
            else if(next_valid_spots[i]==Point(1,1)||next_valid_spots[i]==Point(1,6)||next_valid_spots[i]==Point(6,1)||next_valid_spots[i]==Point(6,6))
            {
                X_num++;
            }
            //mobility
        ...
        if(cur_player==BLACK)
        {
            total+=corner_num*10+mobility_num*3-X_num*10-C_num*10;
        }
    ...
    if(cur_player==BLACK)
    {
        ...
        for(int i=0;i<next_valid_spots.size();i++)
        {
            ...
            tmp.put_disc(next_valid_spots[i]);
            int now=tmp.miniMax(cur_depth+1,depth);
            if(now>max) max=now;
        }
        return max;
    }
